...
class PackageLoader {
  ...
  resolvePackage(packageName: string): string {
    try {
      ...
      const [_, packagePath] = /(^.*node_modules\/[^\/]+)/.exec(mainFilePath)
      return packagePath
    } catch (e) {
      if (e.code === 'MODULE_NOT_FOUND') throw new Error(`No such module ${packageName}`)
      else throw e
    }
  }
  ...
  async loadPackage(packageName: string): Promise<void> {
    ...
    const packagePath = this.resolvePackage(packageName)
      ...
      if (file === path.join(packagePath, 'package.json')) {
        ...
        if (dependencies) {
          await Promise.all(Object.keys(dependencies).map(async (dependency) =>
            await this.loadPackage(dependency)
          ))
        }
      }
    })
