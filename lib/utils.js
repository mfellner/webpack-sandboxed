...
export async function findDirectory(
  root: string,
  name: string,
  ignore: Array<RegExp> = IGNORE_DEFAULT
): Promise<Array<string>> {
  ...
  const fileInfos = await Promise.all(
    files.map(file => path.resolve(root, file)).map(async file => {
  ...
  const isNotIgnored = (file: string) =>
    ignore.reduce((ignore, pattern) => ignore && !pattern.test(file), true);
  ...
  const moreDirs = await Promise.all(
    subDirs.map(async dir => await findDirectory(dir, name))
  );
